from util.tools import visual_sentences
import numpy as np
...
class comprehensive_handler:
    ...
    def bbox_info(self, sample_list, x_min, x_max, y_min, y_max):
        angle_list = []
    ...
    def _fund_minist_box(self, image_name):
        ...
        if len(pred_stage) == 1:
            ...
            x=(stage_record[seleted_box]['x_min']+stage_record[seleted_box]['x_max'])/2
            y=(stage_record[seleted_box]['y_min']+stage_record[seleted_box]['y_max'])/2
            visual_points.append((int(x),int(y)))
            text_list.append(f"[{box_angle_info[seleted_box]['min']}-{box_angle_info[seleted_box]['max']}]({box_angle_info[seleted_box]['mean']})")
            ...
            for label in pred_stage:
                ...
                x=(stage_record[selected_box]['x_min']+stage_record[selected_box]['x_max'])/2
                y=(stage_record[selected_box]['y_min']+stage_record[selected_box]['y_max'])/2
                visual_points.append((int(x),int(y)))
                text_list.append(f"[{box_angle_info[selected_box]['min']}-{box_angle_info[selected_box]['max']}]({box_angle_info[selected_box]['mean']})")
        ...
        visual_sentences(
                        ...
                        box_text=text_list,
                        labels=label_list,
                        save_path=os.path.join(self.visual_dir, image_name)
                    )
        return True
if __name__ == '__main__':
    ...
    handler = comprehensive_handler(model_save_path=os.path.join(args.save_dir, save_model_name), model_config=args.configs['model'])
